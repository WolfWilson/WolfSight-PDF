...
from ui.dialogs import CustomConfirmDialog, SignedResultDialog, PageSelectionDialog
from ui.version import VersionDialog
from utils.resource_handler import resource_path
from utils.validar_firma import find_and_validate_signature # Placeholder import

# ... (Place-holders no modificados) ...
try:
...
class PdfViewer(QWebEngineView):
    ...
    def createWindow(self, _type: QWebEnginePage.WebWindowType) -> "PdfViewer":
        return self
    def load_pdf(self, file_path: str | None) -> None:
        if file_path and os.path.exists(file_path):
            # Añadir un timestamp para forzar la recarga en el cache de webengine
            url = QUrl.fromLocalFile(os.path.abspath(file_path))
            # url.setQuery(f"v={QDateTime.currentMSecsSinceEpoch()}")
            self.load(url)
            self.reload() # Forzar recarga
        else:
            self.setHtml("")
...
class MainWindow(QMainWindow):
    ...
    def _create_widgets(self) -> None:
        # ... (Sin cambios en la creación de widgets del menú) ...
        ...
        menu_map: dict[QPushButton, str] = {
            self.btn_toggle: "Menú", self.btn_open: "Abrir Expediente",
            self.btn_load: "Cargar Documento", self.btn_download: "Descargar",
            self.btn_print: "Imprimir", self.btn_sign: "Firmar Documento",
        }
    ...
    def _create_viewer_container(self, title: str, viewer: QWidget, *, is_annex: bool = False, path_getter: Callable[[], str | None]) -> QWidget:
        # ... (Sin cambios) ...
        container = QWidget()
    ...
    def _get_icon(self, key: str) -> QIcon:
        # ... (Sin cambios) ...
        icon_map = {
            "Menú": "menu.png", "Abrir Expediente": "abrir-expediente.png",
            "Cargar Documento": "cargar-documento.png", "Firmar Documento": "firmar-documento.png",
            "Descargar": "descargar.png", "Imprimir": "imprimir.png",
            "Anexar": "anexar.png", "Cerrar": "cerrar.png",
            "Versión": "wolf.png", "AppIcon": "app_icon.png",
        }
    ...
    def _download_pdf(self, path: str | None) -> None:
        if path: download_pdf(path, self)

    def _print_pdf(self, path: str | None) -> None:
        if path: print_pdf(path, self)
    ...
    def _open_expediente(self) -> None:
        ...
        if path:
            ...
            self.main_header.update_data(Path(path).stem, "TITULAR DEMO")
            if self.content_splitter.sizes()[1] != 0:
                self._close_annex_pane()
    
    # ... (Métodos de anexo sin cambios) ...
    def _load_document_to_annex(self) -> None:
        if not self.current_expediente_path:
            QMessageBox.warning(self, "Acción Requerida", "Primero abra un expediente principal.")
            return
        path, _ = QFileDialog.getOpenFileName(self, "Cargar Documento para Anexar", "", "PDF (*.pdf)")
        if path:
            self.current_annex_path = path
            self.annex_viewer.load_pdf(path)
            self.content_splitter.setSizes([self.width() // 2, self.width() // 2])
            if hasattr(self, "btn_confirm_annex"): self.btn_confirm_annex.setEnabled(True)
    def _confirm_and_annex(self) -> None:
        if not (self.current_expediente_path and self.current_annex_path): return
    ...
    def _close_annex_pane(self) -> None:
        ...
        if hasattr(self, "btn_confirm_annex"): self.btn_confirm_annex.setEnabled(False)

    # ——— firma digital (MODIFICADO) ——————————————————————————————————————————
    def _sign_current_pdf(self) -> None:
        if not self.current_expediente_path:
            QMessageBox.warning(self, "Acción Requerida", "Primero abra un expediente para firmar.")
            return

        src = Path(self.current_expediente_path)
        try:
            reader = PdfReader(src)
            max_pages = len(reader.pages)
        except Exception as e:
            QMessageBox.critical(self, "Error de Lectura", f"No se pudo leer el archivo PDF:\n{e}")
            return

        # 1. Pedir rango de páginas
        page_dialog = PageSelectionDialog(max_pages, self)
        if not page_dialog.exec():
            return
        pages_str = page_dialog.get_selected_pages_str()

        # 2. Pedir certificado PFX
        ...
        pwd, ok = QInputDialog.getText(
            self, "Contraseña", "Contraseña del certificado:", QLineEdit.EchoMode.Password
        )
        if not ok:
            return
        ...
        # 4. Ejecutar firma
        try:
            rec, qr_png = self.signature_manager.create_signed_constancia(
                main_pdf_path=src,
                pages_to_sign_str=pages_str,
                pfx_path=Path(pfx_path),
                pfx_password=pwd,
                user="Wolf-Wilson", # Reemplazar por usuario real
                reason="Documento validado para el proyecto WolfSight-PDF"
            )
        except Exception as exc:
            QMessageBox.critical(self, "Error de Firma", f"La firma falló:\n{exc}")
            return

        # 5. Recargar vista y mostrar resultado
        self.main_viewer.load_pdf(self.current_expediente_path) # Recarga el original estampado
